...
class TestModelPerformance:
    ...
    def setup_test_data(self):
        ...
        # Carregar dataset
        data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'dataset_filtered.csv')
        if not os.path.exists(data_path):
            pytest.skip("Dataset dataset_filtered.csv não encontrado")
        ...
        if not all(col in df.columns for col in required_columns):
            pytest.skip("Dataset não possui colunas necessárias ('Text', 'language')")
        
        # Remover linhas com valores nulos
        df = df.dropna(subset=['Text', 'language'])
        
        # Preparar dados
        X = df['Text'].apply(preprocess_text).values
    ...
    def model(self):
        ...
        model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'best_language_detection_model.pkl')
    ...
    def test_model_confusion_matrix_analysis(self, model, setup_test_data):
        ...
        # Analisar erros
        # Erro tipo 1: Previu Inglês mas era Português
        eng_as_pt = cm[1, 0]
        # Erro tipo 2: Previu Português mas era Inglês
        pt_as_eng = cm[0, 1]
        
        total_samples = cm.sum()
        error_rate_eng_as_pt = eng_as_pt / total_samples
        error_rate_pt_as_eng = pt_as_eng / total_samples
        
        MAX_ERROR_RATE = 0.35
        
        assert error_rate_eng_as_pt <= MAX_ERROR_RATE, \
            f"Taxa de erro (Inglês como Português) de {error_rate_eng_as_pt:.2%} é muito alta (máx: {MAX_ERROR_RATE:.2%})"
        assert error_rate_pt_as_eng <= MAX_ERROR_RATE, \
            f"Taxa de erro (Português como Inglês) de {error_rate_pt_as_eng:.2%} é muito alta (máx: {MAX_ERROR_RATE:.2%})"
